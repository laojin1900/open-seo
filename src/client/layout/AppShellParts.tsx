          ...
          <div className="mx-auto max-w-7xl">
            <div className="alert alert-warning">
              ...
              <span className="text-sm">
                {t("Setup needed: add your DataForSEO API key to use OpenSEO\n                features. See the quick steps on the")}{" "}
                <Link
        ...
        <div className="shrink-0 px-4 py-2.5 md:px-6">
          <div className="mx-auto max-w-7xl">
            <div className="alert alert-info">
              <AlertTriangle className="size-4 shrink-0" />
              <span className="text-sm">
                {t("We could not verify your DataForSEO setup. If features are not\n                working, check the setup steps on the")}{" "}
                <Link
                  {...dataforseoHelpLinkOptions}
                  className="link link-primary font-medium"
                >
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            ...
            >
              {t("One quick setup step")}</h2>
            <p
              id="dataforseo-setup-description"
              className="text-sm text-base-content/75"
            >
              {t("Add your DataForSEO API key to start using OpenSEO.")}</p>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <Link
            ...
            onClick={onClose}
          >
            {t("Open setup guide")}<ExternalLink className="size-4" />
          </Link>
        </div>
      </div>
    </div>
