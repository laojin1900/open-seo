  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-8 p-4 py-8 sm:p-6 md:py-12">
      ...
      <section id="google-analytics" className="space-y-3 scroll-mt-6">
        <h2 className="text-sm font-medium text-base-content/50">{t("Analytics")}</h2>
...
function GeneralSection({ project }: { project: ProjectSummary }) {
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-sm font-medium text-base-content/50">{t("General")}</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-xs text-base-content/50">
            {t("Keyword, SERP, and domain data uses this country and language unless\n            a call asks for a different one.")}</span>
        </div>
      ...
      {confirming ? (
        <div className="space-y-3">
          <p className="text-sm text-base-content/70">
            ...
            {t("removes it from your workspace and stops its scheduled rank\n            tracking. You can restore it later from the Projects page.")}</p>
